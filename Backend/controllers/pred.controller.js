...
const lungpred = asyncHandler(async (req, res) => {
  try {
    ...
    pythonProcess.stderr.on("data", (data) => {
      // TensorFlow logs warnings on stderr even on success
      stderrData += data.toString();
      console.error(`Lung predict stderr: ${data}`);
    });
    ...
    pythonProcess.on("close", (code) => {
      ...
      if (code === 0 && (result === "cancerous" || result === "non-cancerous")) {
        if (result === "cancerous") {
          res
            .status(200)
        ...
        console.error("Lung prediction failed", { code, result, stderrData });
        if (!res.headersSent) {
...
const breastpred = asyncHandler(async (req, res) => {
  ...
  pythonProcess.stderr.on("data", (data) => {
    stderrData += data.toString();
    console.error(`Breast predict stderr: ${data}`);
  });
  ...
  pythonProcess.on("close", (code) => {
    ...
    if (code === 0 && result) {
      ...
      console.error("Breast prediction failed", { code, result, stderrData });
      if (!res.headersSent) {
        res.status(500).json({ error: "Prediction error" });
      }
    }
